...
Produces: `(arrow-polygon [tx ty] [sx sy] w-target w-source) -> [x0 y0 x1 y1 ...]` — vertices of ONE filled polygon, the classic 7-vertex barbed arrow: shaft quad from source (half-width `(max 1 (/ w-source 2))`) to a target-shaft half-width `(max 1 (/ w-target 2))` at the head base (OL width=weight parity), then a head triangle flaring to barb half-width `(max 3 w-target)` at that same head-base station before a point exactly at the target; perimeter order source-left → head-base-neck-left → barb-left → tip → barb-right → head-base-neck-right → source-right. Head length is `(min (max 6 (* 1.8 w-target)) (* 0.6 len))` — the uncapped floor can otherwise place the head base behind the source on short/thick segments (self-intersecting bowtie), so it's capped at 60% of the segment's own length. Segments shorter than 8px skip the head entirely and return a plain 4-vertex tapered quad (source-left → target-left → target-right → source-right, using the source and target-shaft half-widths, no barb). Degenerate zero-length segments return nil. `(point-segment-dist-sq [px py] [ax ay] [bx by]) -> d²` (project-clamp-t formula; endpoint cases when t≤0/t≥1).
Tests: horizontal arrow (assert head vertices at target side, taper widths at correct ends via y-extents at the two x-ends including the barb half-width floor, polygon symmetric about the axis), vertical arrow, zero-length → nil, short-segment quad fallback (no vertex past the source along the axis), the head-length self-intersection clamp on a short/thick segment, dist-sq on-segment/at-endpoints/beyond-ends exact values.
