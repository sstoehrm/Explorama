...
- Theme colors in `styles/src/tailwind.css` are generated from
  `src/scss/base/_colormap.scss` via `styles/scripts/colormap_to_theme.bb.clj`
  — rerun it when the colormap changes and paste the output into the marked
  `BEGIN colormap` / `END colormap` block.
