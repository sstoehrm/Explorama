...
(def ops
  [{:op :projects/current :side :frontend
   ...
   {:op :projects/create :side :frontend
    :doc "Create a project from the current workspace with :title and :description. This is the only way the agent may persist a workspace; there is no save op. Returns :projects/current."
   ...
   {:op :projects/load :side :frontend
    :doc "Load a project into the workspace, replacing what is open. Returns :projects/current after the load started; poll :woco/workspace until the interaction mode is :normal."
    :input [:map {:closed true} [:project-id string?]] :output current :timeout-ms 20000}
   {:op :projects/load-step :side :frontend
    :doc "Replay the loaded project up to protocol step :step. Returns :projects/current."
