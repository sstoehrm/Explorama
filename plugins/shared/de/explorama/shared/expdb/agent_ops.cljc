...
(def ops
  [{:op :expdb/upload :side :backend
   ...
   {:op :expdb/commit :side :backend
    :doc "Commit the staged import into the shared database. Ask the user before calling this. Returns the import summary."
    :input [:map {:closed true}] :output :any :timeout-ms 600000}
   {:op :expdb/cancel :side :backend :doc "Discard the staged import transaction." :input [:map {:closed true}] :output :any}
   {:op :expdb/delete :side :backend :doc "Discard a staged file and its transaction." :input [:map {:closed true} [:file-name [:string {:min 1}]]] :output :any}
