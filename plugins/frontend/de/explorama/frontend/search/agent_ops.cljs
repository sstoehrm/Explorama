...
(defn register! []
  ...
  (registry/register-op! :search/attributes (fn [{:keys [db params ok]}] (ok (attributes db (:frame-id params))) {}))
  (registry/register-op! :search/form-state (fn [{:keys [db params ok]}] (ok (form-state db (:frame-id params))) {}))
  (registry/register-op! :search/data-instance (fn [{:keys [db params ok]}] (ok (data-instance db (:frame-id params))) {}))
  ...
  (registry/register-op! :search/open-visualization
                         (fn [{:keys [params] :as ctx}]
                           (woco-ops/open-vertical (assoc ctx :params {:vertical (:vertical params)
                                                                        :source-frame-id (:frame-id params)
