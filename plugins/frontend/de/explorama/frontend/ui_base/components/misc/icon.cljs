...
(def colors {:white "bg-white"
             :black "bg-black"
             :gray {:default "bg-gray"
                    :0 "bg-black"
                    :1 "bg-gray-900"
                    :2 "bg-gray-800"
                    :3 "bg-gray-700"
  ...
  (let [{:keys [size color custom-color brightness extra-class color-important?] ico :icon}
    ...
    (if-not icon-class
      ...
      [:<>
       ...
       (when sub-icon-class
         [:span {:class (cond-> [sub-icon-class "absolute" "top-2"]
                          color-important? (conj (str color "!"))
