...
(re-frame/reg-event-fx
 ::open-visualization
 (fn [{db :db} [_ indicator-id project? vis-event graph?]]
...
(defn direct-visualization
  ([indicator-id project? disabled?]
   (direct-visualization indicator-id project? disabled? false))
  ([indicator-id project? disabled? graph?]
                ...
                (fn [[key {:keys [event tooltip-search] vertical-icon :icon}]]
                  (let [tooltip-text (if tooltip-search
                    ...
                    {:id (str indicator-id "-" key)
                     ...
                     :on-click (fn []
                                 (re-frame/dispatch [::open-visualization indicator-id project? event graph?]))
